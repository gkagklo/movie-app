      ...
      <nav :class="{'flex': open, 'hidden': !open}"
          ...
          <div @click.away="open = false" class="relative" x-data="{ open: false }">
              <button @click="open = !open"
                  class="{{ request()->routeIs('genre.*') ? 'bg-gray-200 dark:bg-gray-700' : 'bg-transparent dark:bg-transparent' }} flex flex-row items-center w-full px-4 py-2 mt-2 text-sm font-semibold text-left rounded-lg dark:focus:text-white dark:hover:text-white dark:focus:bg-gray-600 dark:hover:bg-gray-600 md:w-auto md:inline md:mt-0 md:ml-4 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline">
                  <span>Genres</span>
              ...
              <div x-show="open" x-transition:enter="transition ease-out duration-100"
                  ...
                  <div class="px-2 py-2 bg-white rounded-md shadow dark:bg-gray-800 flex flex-wrap">
                      @foreach (App\Models\Genre::all() as $genre)
                          <a class=" {{ request()->is('genre/'.$genre->slug) ? 'bg-gray-200 dark:bg-gray-700' : 'bg-transparent dark:bg-transparent' }} text-sm p-2 m-2 font-semibold rounded-lg dark:hover:bg-gray-600 dark:focus:bg-gray-600 dark:focus:text-white dark:hover:text-white dark:text-gray-200 md:mt-0 hover:text-gray-900 focus:text-gray-900 hover:bg-gray-200 focus:bg-gray-200 focus:outline-none focus:shadow-outline"
                              href="{{ route('genre.show', $genre->slug) }}">{{ $genre->title }}</a>
                      @endforeach
                  </div>
              </div>
          </div>
